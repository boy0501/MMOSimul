...
int CPP_Require(lua_State* L)
{
	...
	const char* name = lua_tostring(L, 1);    // require 다음의 파일을 가져온다
...

	{
		...
		sprintf_s(bf, relpaths[i], name);// 상대 경로를 조합하여 경로 생성



		std::string script = bf;
		

		auto err = luaL_loadfile(L, script.c_str());
	...
	lua_call(L, 0, 1);    // 핵심. 이걸 몰라서 2시간 헤맸다.
